...
# Функция hh_get закомментирована, так как HH API не работает (403)
...
# Функция fetch_vacancy_details закомментирована, так как использует hh_get
...
# Функция search_vacancies_api закомментирована, так как использует hh_get и fetch_vacancy_details
...
def generate_fallback_letter(vacancy: Vacancy) -> str:
    ...
    # Ограничиваем количество фраз (не более 4-5, чтобы письмо было лаконичным)
    max_phrases = 5
...
def search_vacancies(
    ...
    """
    Ищет вакансии через браузер (HH API отключён, так как он не работает).
    Параметр headless управляет видимостью браузера.
    """
    # Закомментирован вызов API, так как он выдаёт ошибку 403
    # try:
...
def run_once(config: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    # ШАГ 4: Теперь ищем вакансии (после выбора модели) – всегда через браузер, с учётом headless
